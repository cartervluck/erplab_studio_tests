...
% Save as
    function savecheckedas(source,event)
        ...
        for i = data'
            [in,erp] = ds2erp(i);
            observe_ERPDAT.CURRENTERP = in;
            [observe_ERPDAT.ALLERP(1,in), issave ERPCOM] = pop_savemyerp(erp,'gui','saveas');
            [observe_ERPDAT.ALLERP(1,in), ALLERPCOM] = erphistory(observe_ERPDAT.ALLERP(1,in), ALLERPCOM, ERPCOM);
        end
        ...
        for i = 1:ffnr
            if ismember(cell2mat(datasets(i,2)),ndsns)
                ...
                clear erp
                [in, ~] = ds2erp(data(it,:));
                datasets(i,4) = {observe_ERPDAT.ALLERP(1,in).filename};
                it = it + 1;
            end
        end
...
% Save
    function savechecked(source,event)
        ...
        for i = data'
            [in,erp] = ds2erp(i);
            observe_ERPDAT.CURRENTERP = in;
            [observe_ERPDAT.ALLERP(1,in), issave ERPCOM] = pop_savemyerp(erp,'gui','save');
            [observe_ERPDAT.ALLERP(1,in), ALLERPCOM] = erphistory(observe_ERPDAT.ALLERP(1,in), ALLERPCOM, ERPCOM);
        end
...
% Append ERPsets
    function appenddata(source,event)
        ...
        pop_appenderp(observe_ERPDAT.ALLERP,'Erpsets',einds);
...
% On ERPset selected set ERP, CURRENTERP
    function selectdata(source,event)
        ...
        if source.BackgroundColor ~= [0.1 0.25 0.5]
            i = 0;
            ...
            for child = i.Children'
                if strcmp(child.Style,'text')
                    if all(ismember(child.String, '0123456789'))
                        selectedData = str2double(child.String);
                        clear k
                        for k = datasets'
                            if cell2mat(k(2)) == str2double(child.String)
                                [cind, e] = ds2erp(k);
                                observe_ERPDAT.CURRENTERP = cind;
                                break
                            end
                        end
                        break
    ...
    function duplicateSelected(source,event)
        ...
        for i = datasets'
            ...
            if ismember(j,ndsns)
                ...
                clear k
                for k = observe_ERPDAT.ALLERP
                    if strcmp(k.filepath,cell2mat(datasets(end,5))) && strcmp(k.filename,cell2mat(datasets(end,4)))
                        observe_ERPDAT.ALLERP(1,end+1) = k;
                        observe_ERPDAT.ALLERP(1,end).erpname = cell2mat(datasets(end,1));
                        observe_ERPDAT.ALLERP(1,end).filename = strcat(observe_ERPDAT.ALLERP(1,end).filename(1:end-4),'_copy.erp');
        ...
        for i = 1:r
            if ismember(cell2mat(datasets(i,2)),ndsns)
                ...
                try
                    ...
                    [~,cerp] = size(observe_ERPDAT.ALLERP);
                    for k = 1:cerp
                        if strcmp(observe_ERPDAT.ALLERP(1,k).filepath,cell2mat(datasets(i,5))) && strcmp(observe_ERPDAT.ALLERP(1,k).filename,cell2mat(datasets(i,4)))
